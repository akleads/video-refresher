...
<task type="auto">
  ...
  <action>
    ...
    Important details:
    - Use the same multer `upload` middleware already imported -- it accepts video/mp4 and video/quicktime MIME types, which works since the blobs are video/mp4.
    - However, the multer fileFilter currently only accepts video/mp4 and video/quicktime. The browser will send the blobs with a generic type. To handle this, use a SEPARATE multer instance for the device endpoint that does NOT filter by MIME type (since these are pre-processed results, not raw uploads). Create it inline: `multer({ storage: multerStorage, limits: { fileSize: 500*1024*1024, files: 200 } })` using the same storage config but without fileFilter. Import multer and configure storage at the top of the route file, or create a small helper. Actually, simpler approach: create a second multer config in middleware/upload.js that exports `deviceUpload` without a fileFilter, reusing the same storage.
