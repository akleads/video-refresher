...
<task type="auto">
  ...
  <action>
    ...
    **Module state:**
    ...
    - `pendingFiles` (Array<File> or null) — set by `setDeviceProcessingData()`, consumed by `renderDeviceProgress()`
    - `pendingVariationCount` (number or null) — set by `setDeviceProcessingData()`, consumed by `renderDeviceProgress()`

    **renderDeviceProgress():**
    - Read files and variationCount from module state: `const files = pendingFiles; const variationCount = pendingVariationCount;`
    - If `pendingFiles` is null or empty, render error message "No files to process. Go back to upload." with a link to `#upload`. Return early.
...
<task type="auto">
  ...
  <verify>
    ...
    - Navigating to `#device-progress` in browser shows the view container and calls renderDeviceProgress (renders "No files to process" message when no data set)
  </verify>
  <done>
