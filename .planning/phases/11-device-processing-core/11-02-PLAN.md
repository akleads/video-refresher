...
<task type="auto">
  ...
  <action>
    ...
    **Class: WorkerPool**
    ...
    - Read the File as ArrayBuffer using `file.arrayBuffer()`.
    - Store callbacks.
    - Create a job for each effect: `{ videoData: Uint8Array (copy each time!), effect, outputName: string, retries: 0, resolve, reject }`.
    ...
    - `type: 'error'`: If `job.retries < 1`, increment retries, re-create videoData copy, push back to front of queue. Else, mark as failed (resolve with null instead of reject -- to not abort the batch), call `onVariationComplete` with failed status. Mark worker not busy, call `processNext()`.
