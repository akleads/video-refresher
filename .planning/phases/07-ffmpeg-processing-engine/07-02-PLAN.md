...
<task type="auto">
  ...
  **`processFile(file, job, variationsPerVideo, db, queries, outputDir)`** (internal, not exported):
  ...
  7. The `onProgress` callback should compute overall file progress:
     - Each variation contributes `100 / variationsPerVideo` to total progress
     - For variation i at p%: `overallProgress = Math.round((i * 100 + p) / variationsPerVideo)`
     - Throttle DB writes: only update if progress changed by >= 2 since last write
     - `queries.updateFileProgress.run(overallProgress, file.id)`
