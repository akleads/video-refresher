...
<task type="auto">
  ...
  **1. Import and start queue worker:**
  ...
  ```
  After database initialization, create and start the worker. **Note:** `OUTPUT_DIR` is already defined in server/index.js (line 16): `const OUTPUT_DIR = process.env.OUTPUT_DIR || '${DATA_DIR}/output';` -- no new constant needed.
  ```javascript
  ...
  **2. Startup recovery function (`recoverInterruptedJobs`):**
  ...
  function recoverInterruptedJobs(db, queries) {
    ...
    //    NOTE: Uses SIGKILL (not SIGTERM) because these are orphaned processes from a
    //    previous server run -- they have no parent managing them and their output is
    //    already invalid. In contrast, gracefulShutdown() uses SIGTERM first (with a
    //    2-second grace period before SIGKILL) because the current server is still
    //    running and can wait for FFmpeg to clean up properly.
    const filesWithPid = queries.getFilesWithPid.all();
    for (const file of filesWithPid) {
      try {
        // Check if process is still alive
        process.kill(file.ffmpeg_pid, 0);
        // If we get here, process is alive -- kill it immediately (orphaned, no grace needed)
