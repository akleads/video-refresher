...
<task type="auto">
  ...
  <files>server/db/schema.js, server/db/index.js, server/db/queries.js</files>
  <action>
  ...
  **job_files additions (ALTER TABLE with IF NOT EXISTS pattern):**
  ...
  - `updated_at TEXT NOT NULL DEFAULT (datetime('now'))` -- timestamp for last status/progress update
  ...
  **Important:** Since SQLite does not support `ALTER TABLE ... ADD COLUMN IF NOT EXISTS`, define and export a `migrateSchema(db)` function **in `server/db/schema.js`** (alongside `createTables`). It must be called from `server/db/index.js` immediately after `createTables(db)`:

  ```javascript
  // In server/db/index.js, add:
  import { createTables, migrateSchema } from './schema.js';
  // ... then after createTables(db):
  migrateSchema(db);
  ```

  The `migrateSchema(db)` function structure in `server/db/schema.js`:
  ```javascript
  ...
  This is idempotent: running it multiple times is safe. Also update `files_modified` in the frontmatter to include `server/db/index.js` (adding the migrateSchema import).
  ...
  Note: `updated_at` is included in the migration column list above and will be added via `migrateSchema()`. It is NOT in the Phase 6 schema.
  ...
  <verify>
  Run `node -e "import('./server/db/index.js').then(m => { const db = m.initDatabase('./data/test-schema.db'); import('./server/db/queries.js').then(q => { const queries = q.createJobQueries(db); console.log('All queries prepared successfully'); console.log('Tables:', db.prepare(\"SELECT name FROM sqlite_master WHERE type='table'\").all()); db.close(); }); })" && rm -f ./data/test-schema.db` from the project root. Should print all table names including `output_files` and "All queries prepared successfully" without errors. Also run it twice to verify migration idempotency (second run should also succeed without errors).
  </verify>
  <done>
...
<success_criteria>
- Schema has 6 new columns on job_files (progress_percent, duration_seconds, ffmpeg_pid, error, completed_variations, updated_at) + new output_files table with indexes
