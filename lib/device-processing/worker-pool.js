...
export class WorkerPool {
  ...
  async processVideo(videoFile, effects, onProgress, onVariationComplete) {
    ...
    const jobPromises = effects.map((effect, idx) => {
      return new Promise((resolve, reject) => {
        const job = {
          videoData: new Uint8Array(this.originalBuffer), // Fresh copy
          effect,
    ...
    switch (type) {
      ...
      case 'error': {
        ...
        if (job.retries < 1) {
          ...
          // Create fresh copy of video data from stored buffer
          job.videoData = new Uint8Array(this.originalBuffer);
  ...
  handleError(workerIdx, error) {
    ...
    if (job.retries < 1) {
      ...
      // Create fresh copy of video data
      job.videoData = new Uint8Array(this.originalBuffer);
