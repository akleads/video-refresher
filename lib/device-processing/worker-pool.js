...
export class WorkerPool {
  ...
  async processVideo(videoFile, effects, onProgress, onVariationComplete) {
    ...
    // Extract original file extension for FFmpeg container detection
    const videoExt = '.' + videoFile.name.split('.').pop().toLowerCase();
    this.currentVideoExt = videoExt;
    ...
    const jobPromises = effects.map((effect, idx) => {
      return new Promise((resolve, reject) => {
        const job = {
          effect,
          outputName: `${videoBaseName}/${videoBaseName}_var${idx + 1}_${crypto.randomUUID().slice(0, 8)}.mp4`,
          retries: 0,
          variationIndex: idx,
          resolve,
          reject
  ...
  handleMessage(workerIdx, event) {
    ...
    switch (type) {
      case 'complete': {
        ...
        this.progressTracker.markComplete();
